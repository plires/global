  ...
  public function setEmailRecipients($objectPhpMailer, $recipient, $post, $destinationEmail)
  {
    ...
    switch ($recipient) {
      ...
      case 'Cliente':
        ...
        if (!empty($_ENV['EMAIL_RECIPENT_BCC'])) {
          $bccList = explode(',', $_ENV['EMAIL_RECIPENT_BCC']);
          foreach ($bccList as $bcc) {
            $objectPhpMailer->addBCC(trim($bcc));
          }
        }
  ...
  public function setTemplateAndEmailSubject($template, $post, $destinationEmail)
  {
    ...
    switch ($template) {
      ...
      case 'Contacto Cliente':
        ...
        $email['subject'] = 'Nuevo Consulta desde ' . $_ENV['NAME_APP'];
        break;
